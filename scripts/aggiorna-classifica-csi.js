...
async function main(){
  ...
  $('table').each((_, table) => {
    ...
    if (!/\bPt\b/i.test(headerText)) return; // non è la tabella classifica
    $table.find('tbody tr').each((__, tr) => {
      const cols = $(tr).find('td').map((___, td) => $(td).text().trim()).get();
      if (cols.length < 9) return;
      const [, squadra, pt, pg, v, n, p, gf, gs] = cols;
      if (!squadra) return;
      righeClassifica.push({
        squadra: normTeam(squadra),
